  ...
  return (
   <React.Fragment>
     <div className=" flex justify-between h-52 bg-[url('https://cdn.wallpapersafari.com/81/23/w2C6fu.jpg')]">
      <div className="z-10 mr-96 mx-4 my-2 w-60 h-96"><Link to="/Songlist"><button onClick={() => filterForm()} className="z-0 py-2 h-10 w-20 border-2 border-double hover:bg-slate-800 font-bold border-green-500 rounded-xl text-white" type="button">Filter</button></Link> 
        {value ? (
      ...
      <div className="mr-40 self-center">
        <h1 className="text-4xl">THIS IS SOME COOL QUOTE</h1>
        <p className="text-xl">A really really cool paragraph lol lorem lorem lorem lorem</p>
      </div>
    </div>
   </React.Fragment>
   )
